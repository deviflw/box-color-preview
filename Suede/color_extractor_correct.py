...
def get_grid_config(image_file):
    ...
    if "suede-1.jpg" in image_file:
        return 5, 6  # 5 rows, 6 columns = 30 colors
    else:
        return 5, 10  # 5 rows, 10 columns = 50 colors
...
def process_image_correct(image_path, starting_number):
...
def generate_html_grid(all_colors_data, output_file='extracted_colors_correct.html'):
    """Generate HTML with correct grid layouts and sequential numbering."""
...
<head>
    ...
    <title>Suede Color Collection - Sequential Numbering</title>
    <style>
        body {{
...
"""
    ...
    for image_name, colors in all_colors_data.items():
        ...
        if num_colors == 30:
            grid_layout = "5×6 grid"
        else:
            grid_layout = "5×10 grid"
...
"""
        ...
        cols = 6 if num_colors == 30 else 10
...
def main():
    ...
    print(f"Found {len(image_files)} images to process with correct grid configurations...")
    ...
    for image_file in ordered_files:
        ...
        colors_data, colors_count = process_image_correct(image_path, current_color_number)
    ...
    if all_colors_data:
        # Generate HTML
        output_html = os.path.join(input_folder, 'extracted_colors_correct.html')
        generate_html_grid(all_colors_data, output_html)
        
        # Save JSON
        json_output = os.path.join(input_folder, 'extracted_colors_correct.json')
