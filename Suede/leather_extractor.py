...
def main():
    ...
    # Look for leather image file
    image_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff')
    leather_files = [f for f in os.listdir(input_folder) 
                     if f.lower().endswith(image_extensions) and 'leather' in f.lower()]
    
    if not leather_files:
        print("No leather image file found. Looking for any image files...")
        all_image_files = [f for f in os.listdir(input_folder) 
                          if f.lower().endswith(image_extensions)]
        print(f"Available image files: {all_image_files}")
        return
    
    leather_file = leather_files[0]  # Take the first leather file found
