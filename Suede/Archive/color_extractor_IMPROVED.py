...
def create_improved_grid(image, rows, cols):
    """
    Create a grid with SHRUNK sampling areas to avoid borders.
    This samples from the CENTER of each grid cell to avoid white borders.
    """
    ...
    # Define border percentage to avoid (e.g., 20% from each side)
    border_percent = 0.20
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if new_w > 20 and new_h > 20:
                ...
                # Fallback to original if too small
                squares.append((orig_x, orig_y, orig_w, orig_h))
    
    return squares

def process_image_improved(image_path, starting_number):
    """Process image with IMPROVED sampling to avoid borders."""
    ...
    print(f"  IMPROVED: Sampling from CENTER of each cell to avoid borders")
    print(f"  Color numbers: {starting_number:03d} to {starting_number + total_squares - 1:03d}")
    
    # Create IMPROVED grid (avoids borders)
...
def generate_html_grid(all_colors_data, output_file='extracted_colors_IMPROVED.html'):
    """Generate HTML with improved color extraction."""
...
<head>
    ...
    <title>Suede Color Collection - IMPROVED Extraction</title>
    <style>
        body {{
...
<body>
    ...
    <div class="summary">
        <h1>Suede Color Collection - IMPROVED</h1>
        <p>Enhanced extraction avoiding borders - Sequential numbering 001 to {total_colors:03d}</p>
        <div class="stats">
            <div class="stat">
    ...
    <div class="improvement-note">
        <strong>IMPROVEMENT:</strong> Now sampling from the CENTER of each grid cell to avoid white borders and empty areas. This should capture the actual color swatches instead of background areas.
    </div>
"""
    ...
    <div class="image-section">
        ...
        <div class="grid-info">
            ✓ IMPROVED: {grid_layout} - Colors {first_color} to {last_color} ({num_colors} colors)
            <br>Sampling from CENTER of each cell to avoid borders
        </div>
...
def main():
    ...
    print("IMPROVEMENT: Now sampling from CENTER of each grid cell to avoid borders!")
    ...
    for image_file in ordered_files:
        ...
        colors_data, colors_count = process_image_improved(image_path, current_color_number)
        
        if colors_data:
            all_colors_data[image_file] = colors_data
            current_color_number += colors_count
            print(f"  SUCCESS: Extracted {colors_count} colors with IMPROVED sampling")
        else:
            print(f"  FAILED to process {image_file}")
    
    if all_colors_data:
        # Generate HTML
        output_html = os.path.join(input_folder, 'extracted_colors_IMPROVED.html')
        generate_html_grid(all_colors_data, output_html)
        
        # Save JSON
        json_output = os.path.join(input_folder, 'extracted_colors_IMPROVED.json')
        ...
        print("IMPROVED: Should now have much better color extraction!")
    else:
